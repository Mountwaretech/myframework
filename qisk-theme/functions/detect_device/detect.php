...
class deviceDetect {
	...
	public static function deviceType() {
		return (self::$detect->isMobile() ? (self::$detect->isTablet() ? 'mobile' : 'mobile') : 'computer');
	}
	...
	public static function os() {
        ...
        if (preg_match('/android/i', $agent)) {
			...
			$os = array("os"=>"android","name"=>'Android' . $version . $codeName);
		} elseif (preg_match('/Linux/i', $agent)) {
			  $os = array("os"=>"linux");
		} elseif (preg_match('/Mac OS X/i', $agent)) {
            $os_d="Mac OS";

            if (preg_match('/Mac OS X 10_11/', $agent) || preg_match('/Mac OS X 10.12/', $agent)) {
				$os_d = 'OS X (Sierra)';
			} 
           elseif (preg_match('/Mac OS X 10_11/', $agent) || preg_match('/Mac OS X 10.11/', $agent)) {
				$os_d = 'OS X (El Capitan)';
		    ...
		    $os = array("os"=>"windowsphone","name"=>'Windows Phone' . $version);
			...
			switch (self::$detect->version('Windows NT')) {
				...
				default: $codeName = ' NT v' . self::$detect->version('Windows NT'); break;
			     ...
			     $os = array("os"=>"windows","name"=>'Windows' . $codeName);
		}
		return $os;
...
class Debug {
 ...
 $GLOBALS["device"]=new deviceDetect();
